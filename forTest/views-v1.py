...
def upload(request):
	...
	if request.method == 'POST':
		...
		if docform.is_valid():
			...
			vp = docform.cleaned_data['vehicle_platform']
			code = docform.cleaned_data['ps_platform']
			my = docform.cleaned_data['my']
			phase = docform.cleaned_data['phase']		
			filename = docform.cleaned_data['docfile'].name.split('.')[0].replace(" ","_")
			fileformat = docform.cleaned_data['docfile'].name.split('.')[-1]
			ftime = filename.split('-')[-1]
			...
			if fileformat == 'xlsx':
				...
				fpath = 'D:/DjangoWeb/myproject/media/documents/' + filename
