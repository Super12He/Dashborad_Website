...
def detail(request):
	...
	for line in csv_reader:
		...
		tableContent.append('''<td><a data-toggle="modal" href="#myModal" class="glyphicon glyphicon-trash"></a></td>\n''')
		tableContent.append('</tr>\n')
...
def upload(request,template='upload.html'):
	...
	# if request.method == 'POST':
		...
		# for files in filelist:
			...
			# if dform.is_valid():
				# newdoc = handle_uploaded_file('file')

				# filename = dform.cleaned_data['docfile'].name.split('.')[0]
				# fileformat = dform.cleaned_data['docfile'].name.split('.')[-1]
				# name = dform.cleaned_data['your_name']
	...
	# return render(
		...
		# 'upload.html',
		# {'form': dform}
		# {'documents': documents, 'form': dform}
	# )
