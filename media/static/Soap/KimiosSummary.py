    ...
    with open(fileNameTable, 'a', encoding="utf-8") as fout:
        for item in sink:
            ...
            if len(relatedDocument) != 0:
                try:
                    itemstr = [str(item.owner).replace(' Archive', '').strip(), item.name + '.' + item.extension, relatedDocument.Document[0].name + '.' + relatedDocument.Document[0].extension]
                ...
                itemstr = [str(item.owner).replace(' Archive', '').strip(), item.name + '.' + item.extension, 'None']
                itemstr.extend(queryString.replace('Report:', '').replace(' ','').split('&'))
                fout.write('%s, %s, %s, %s, %s, %s\n' % tuple(itemstr))
...
for target in targets:
    ...
    for meta in metas:
        for phase in phases:
            ...
            reportGenerator(queryString, advancedRetrieve(queryGenerator(target, [meta, phase])))
with open(fileName, 'a', encoding="utf-8") as fout:
